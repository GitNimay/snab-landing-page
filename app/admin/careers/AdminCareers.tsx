  ...
  return <main className="admin-careers">
    ...
    <div className="admin-workspace">
      ...
      {activeView === "applications" ? <>
        ...
        return <article key={job.id} className={job.status === "deleted" ? "job-admin-deleted" : ""}>
          <div>
            ...
            <span className={`job-deadline ${expired ? "is-expired" : ""}`}><CalendarClock size={13} />{deadlineLabel(job)}</span>
          </div>
    ...
    {jobToDelete ? <div className="admin-modal-scrim"><section className="delete-job-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-job-title"><span><AlertTriangle /></span><p>Delete job posting</p><h2 id="delete-job-title">Remove {jobToDelete.title}?</h2><p>This removes the role from the careers site immediately. Existing applications and candidate history will remain available in this dashboard.</p><div><button onClick={() => setJobToDelete(null)}>Keep posting</button><button className="confirm-delete" onClick={deleteJob} disabled={loading}>{loading ? "Deleting…" : "Delete posting"}</button></div></section></div> : null}
  </main>;
}
