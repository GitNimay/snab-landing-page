...
export function AdminCareers() {
  ...
  return <main className="admin-careers">
    ...
    <div className="admin-workspace">
      ...
      </> : <section className="jobs-admin-list">{jobs.map((job) => <article key={job.id}><div><span className={`job-status job-status-${job.status}`}>{job.status}</span><p>{job.department}</p><h2>{job.title}</h2><small>{job.location} · {job.work_mode} · {job.employment_type}</small></div><div className="job-admin-count"><strong>{applications.filter(item => item.job_id === job.id).length}</strong><span>applications</span></div><button onClick={() => editJob(job)}>Edit posting</button></article>)}</section>}
    </div>
